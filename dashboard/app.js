...
function activateView(viewName) {
  document.querySelectorAll(".metric-page").forEach((page) => {
...
function setupTabs() {
  ...
  const initialView = window.location.hash.replace("#", "") || "direct";
  activateView(initialView === "cumulative" ? "cumulative" : "direct");
}
...
function fillDirectTable(readings) {
  const rows = document.getElementById("directRows");
  rows.innerHTML = "";
  readings.slice(-6).reverse().forEach((reading) => {
    const row = document.createElement("tr");
    row.innerHTML = `
      <td>${formatTime(reading.timestamp)}</td>
      <td>${watt.format(reading.power_w)} W</td>
      <td>${Number(reading.temperature_c || 0).toFixed(1)} C</td>
      <td>${reading.data_source || "unbekannt"}</td>
    `;
    rows.appendChild(row);
  });
}
...
async function loadDashboard() {
  ...
  setText("plantNameCumulative", latest.plant_id || "keine Anlage");
  ...
  setText("temperatureCard", `${Number(latest.temperature_c || 0).toFixed(1)} C`);
  setText("sourceCard", latest.data_source || "unbekannt");
  setText("readingCount", `${summary.reading_count || readings.length} Werte`);
  setText("readingCountCard", `${summary.reading_count || readings.length}`);
  setText("latestTime", latest.timestamp ? formatTime(latest.timestamp) : "--");
  setText("readingType", summary.latest_is_test_data ? "TESTDATEN" : "ECHTDATEN");
